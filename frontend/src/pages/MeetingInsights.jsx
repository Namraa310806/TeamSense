...
function MeetingInsights() {
  ...
  const sentimentColor = meeting?.sentiment_score > 0.6 ? 'text-accent-emerald' : meeting?.sentiment_score > 0.4 ? 'text-accent-amber' : 'text-accent-rose';

  return (
    <div className="max-w-5xl mx-auto space-y-6">
      {/* Back */}
      <Link to="/employees" className="flex items-center gap-2 text-slate-400 hover:text-white text-sm transition-colors">
      ...
      <div className="glass-card p-6 animate-slide-up">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white">Meeting Insights</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card p-6 animate-fade-in">
          ...
          <div className="text-sm text-slate-300 leading-relaxed whitespace-pre-line bg-surface-900/50 rounded-xl p-4 border border-surface-700/50">
            ...
            <div className="space-y-3">
              {(meeting?.key_topics || []).map((topic, i) => (
                <div key={i} className="flex items-center justify-between">
                  <span className="text-sm text-slate-300">{topic.topic || topic}</span>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500">{((topic.relevance || 0.5) * 100).toFixed(0)}%</span>
                  </div>
                </div>
              ))}
              {(!meeting?.key_topics || meeting.key_topics.length === 0) && (
                <p className="text-slate-500 text-sm">No topics extracted</p>
              )}
            </div>
          </div>
          ...
          <div className="glass-card p-6 animate-fade-in">
            ...
            <div className="space-y-3">
              <div className="sentiment-bar">
                <div
                  className="absolute top-1/2 transform -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow-lg border-2 border-primary-500 transition-all"
      ...
      <div className="glass-card p-6 animate-fade-in">
        ...
        <div className="text-sm text-slate-400 leading-relaxed bg-surface-900/50 rounded-xl p-4 border border-surface-700/50 max-h-96 overflow-y-auto">
